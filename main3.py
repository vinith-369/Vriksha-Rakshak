...
# Language Selection
language = st.sidebar.selectbox("Select Language", ["English", "Spanish", "French", "German", "Chinese", "Hindi", "Tamil", "Telugu", "Bengali", "Gujarati"])
...
st.markdown(
    ...
    .stApp {
        ...
        background-attachment: fixed;  /* Makes gradient fixed during scrolling */
    }
    ...
    [data-testid="stSidebar"] {
        ...
        border-right: 1px solid #ffffff20;  /* Semi-transparent white border */
    }
    
    /* Make sidebar content transparent to show gradient */
    .sidebar .sidebar-content {
        background: transparent !important;
    }
    
    /* Sidebar text color - force white for all elements */
    # [data-testid="stSidebar"] * {
    ...
    h1 {
        ...
        border-bottom: 2px solid #3CB371;  /* Underline effect */
        padding-bottom: 0.3em;  /* Space between text and underline */
    }
    
    /* Secondary header (h2) styling */
    h2 {
        color: #3CB371;  /* Medium sea green */
    }
    
    /* Tertiary header (h3) styling */
    h3 {
    ...
    /* Main button styling */
    .stButton>button {
        background: linear-gradient(135deg, #3CB371 0%, #2E8B57 100%);  /* Green gradient */
        color: white;
        border: none;
    ...
    .card {
        ...
        border-left: 4px solid #3CB371;  /* Accent border on left */
    }
    
    /* ======================== */
    /* INPUT FIELDS */
    /* ======================== */
    /* Style text inputs, number inputs, and select boxes */
    ...
    .stTabs [aria-selected="true"] {
        background: #3CB371 !important;  /* Green background */
        color: white !important;
    }
    ...
    [data-testid="stFileUploader"] {
        border: 2px dashed #3CB371;  /* Dashed green border */
        border-radius: 12px;  /* Rounded corners */
        padding: 2rem;  /* Internal spacing */
    }
    ...
    @keyframes fadeIn {
        from { opacity: 0; transform: translateY(10px); }  /* Start slightly transparent and lower */
        to { opacity: 1; transform: translateY(0); }  /* End fully visible in normal position */
    }
    
    /* Class to apply fade-in animation */
    .animated {
        animation: fadeIn 0.5s ease-out;  /* 0.5 second animation with ease-out */
    }
    ...
    [data-testid="stSidebarNav"] a:hover {
        background-color: #e6ffeb !important;  /* Light green hover background */
        color: #2E8B57 !important;  /* Dark green text on hover */
    }
...
if app_mode == "Home":
    st.sidebar.markdown(translate_text("""🏠 *Home:* Overview of the app and main features.""",lang_code))
elif app_mode == "Disease Recognition":
    st.sidebar.markdown(translate_text("""🦠 *Disease Recognition:* Identify plant diseases using images.""",lang_code))
    st.sidebar.markdown(translate_text("""Upload photos of plant leaves to get instant diagnosis.""",lang_code))
elif app_mode == "Crop Recommendation":
    st.sidebar.markdown(translate_text("""🌾 *Crop Recommendation:* Get suggestions on what to plant based on your conditions.""",lang_code))
    st.sidebar.markdown(translate_text("""Enter soil parameters to find the perfect crop match.""",lang_code))
elif app_mode == "Insect Detection":
    st.sidebar.markdown(translate_text("""🐛 *Insect Detection:* Identify harmful insects affecting your crops.""",lang_code))
    st.sidebar.markdown(translate_text("""Upload insect images for identification and control methods.""",lang_code))
elif app_mode == "Weather":
    st.sidebar.markdown(translate_text("""🌦 *Weather:* Check the local weather for better farming decisions.""",lang_code))
    st.sidebar.markdown(translate_text("""Real-time weather data and forecasts for your location.""",lang_code))
elif app_mode == "Organic Farming":
    st.sidebar.markdown(translate_text("""🌍 *Organic Farming:* Explore eco-friendly farming practices and sustainable solutions.""",lang_code))
    st.sidebar.markdown(translate_text("""Step-by-step guides for growing crops naturally.""",lang_code))
elif app_mode == "Soil Health":
    st.sidebar.markdown(translate_text("""🌱 *Soil Health:* Learn about soil quality and health indicators.""",lang_code))
    st.sidebar.markdown(translate_text("""AI-powered soil analysis and improvement recommendations.""",lang_code))
elif app_mode == "About":
    st.sidebar.markdown(translate_text("""📖 *About:* Learn more about the project and its objectives.""",lang_code))
    st.sidebar.markdown(translate_text("""Our mission and the technology behind Vriksha Rakshak.""",lang_code))
...
if app_mode == "Home":
    ...
    with col1:
        ...
        <p>{translate_text("Your all-in-one farming assistant powered by artificial intelligence to help you grow better crops with less effort.",lang_code)}</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        <h3>Key Features</h3>
        <ul>
          <li>🌿 {translate_text("Identify plant diseases with image recognition",lang_code)}</li>
          <li>🌾 {translate_text("Get personalized crop recommendations",lang_code)}</li>
          <li>🐛 {translate_text("Detect harmful insects and get control methods",lang_code)}</li>
          <li>⛅ {translate_text("Check hyperlocal weather forecasts",lang_code)}</li>
          <li>🌱 {translate_text("Learn organic farming techniques",lang_code)}</li>
          <li>🧪 {translate_text("Analyze your soil health",lang_code)}</li>
        </ul>
        </div>
        """, unsafe_allow_html=True)
    
    with col2:
        st.image("home_page1.jpg", use_column_width=True)
        
        st.markdown(f"""
        <div class="card">
        <h3>Getting Started</h3>
        <ol>
          <li>{translate_text("Select a feature from the sidebar",lang_code)}</li>
          <li>{translate_text("Provide required inputs (images, parameters, etc.)",lang_code)}</li>
          <li>{translate_text("Get instant AI-powered recommendations",lang_code)}</li>
           <li>{translate_text("Implement the suggestions for better yields",lang_code)}</li>
        </ol>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        <ul>
          <li>{translate_text("Take photos in natural daylight",lang_code)}</li>
          <li>{translate_text("Focus on affected leaves",lang_code)}</li>
          <li>{translate_text("Try to add close-up views",lang_code)}</li>
          <li>{translate_text("Ensure the image is clear and in focus",lang_code)}</li>
    ...
    with col2:
        test_image = st.file_uploader("Choose an image file", type=["jpg", "jpeg", "png"], 
                                     help=translate_text("Upload a clear image of the affected plant part",lang_code))
    ...
    if test_image:
        ...
        with col2:
            if st.button(translate_text("🔍 Analyze Plant",lang_code), help=translate_text("Click to predict disease",lang_code)):
                with st.spinner("Analyzing your plant image... Please wait."):
                    ...
                    <p style="font-size: 1.2rem; font-weight: bold; color: #2E8B57;">{translate_text(class_name[result_index],lang_code)}</p>
                    </div>
                    
                    <div style="padding: 1rem; background-color: white; border-radius: 8px;">
                    <h4>Recommended Treatment:</h4>
                    <p>{translate_text(plant_disease_treatments[class_name[result_index]],lang_code)}</p>
                    </div>
                    </div>
                    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div class="card">
    <h1>{translate_text("Soil Health Analysis", lang_code)}</h1>
    <p>{translate_text("Get AI-powered soil quality assessment and improvement recommendations.", lang_code)}</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown(f"""
        <div class="card">
        <h3>{translate_text("Crop History", lang_code)}</h3>
        """, unsafe_allow_html=True)
        crop_year_1 = st.text_input(translate_text("Year 1 Crop", lang_code), placeholder="Wheat")
        crop_year_2 = st.text_input(translate_text("Year 2 Crop", lang_code), placeholder="Soybean")
        crop_year_3 = st.text_input(translate_text("Year 3 Crop", lang_code), placeholder="Corn")
        st.markdown("</div>", unsafe_allow_html=True)

        st.markdown(f"""
        <div class="card">
        <h3>{translate_text("Soil Properties", lang_code)}</h3>
        """, unsafe_allow_html=True)
        soil_texture = st.selectbox(
            translate_text("Soil Texture", lang_code), 
            [translate_text(x, lang_code) for x in [
        ...
        soil_ph = st.slider(
            translate_text("Soil pH", lang_code),
            0.0, 14.0, 6.5, 0.1,
            help=translate_text("Ideal range for most crops: 6.0-7.0", lang_code)
        )
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with col2:
        st.markdown(f"""
        <div class="card">
        <h3>{translate_text("Farming Practices", lang_code)}</h3>
        """, unsafe_allow_html=True)
        tillage_practices = st.selectbox(
            translate_text("Tillage Practices", lang_code), 
            [translate_text(x, lang_code) for x in ["No-till", "Reduced tillage", "Conventional tillage"]]
        )
        fertilization_practices = st.selectbox(
            translate_text("Fertilization", lang_code), 
            [translate_text(x, lang_code) for x in ["Organic", "Inorganic", "Both", "None"]]
        )
        irrigation_practices = st.selectbox(
            translate_text("Irrigation", lang_code), 
            [translate_text(x, lang_code) for x in ["Drip", "Sprinkler", "Flood", "Furrow", "None"]]
        )
        st.markdown("</div>", unsafe_allow_html=True)

        st.markdown(f"""
        <div class="card">
        <h3>{translate_text("Location", lang_code)}</h3>
        """, unsafe_allow_html=True)
        ...
        state_name = st.selectbox(translate_text("Select State", lang_code), options=states)
        city_name = ""
        if state_name:
            cities = data[state_name]
            city_name = st.selectbox(translate_text("Select City", lang_code), options=cities)

        st.markdown("</div>", unsafe_allow_html=True)

    # Analyze Button
    if st.button("🔍 " + translate_text("Analyze Soil Health", lang_code), help=translate_text("Click to get soil health assessment", lang_code)):
        if crop_year_1 and crop_year_2 and crop_year_3:
            with st.spinner(translate_text("Analyzing soil health... Please wait.", lang_code)):
                prompt = f"""
                ...
                try:
                    ...
                    <h2>{translate_text("Soil Health Report", lang_code)}</h2>
                    <div style="padding: 1rem; background-color: white; border-radius: 8px; margin: 1rem 0;">
                    <p>{response.text}</p>
                    </div>
                    </div>
            ...
            st.warning(translate_text("Please provide crop history for all three years.", lang_code))
    ...
    st.markdown(f"""
    <div class="card">
    <h1>{translate_text("Insect Detection & Management",lang_code)}</h1>
    <p>{translate_text("Upload insect images for identification and get control recommendations.",lang_code)}</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown(f"""
        <div class="card">
        <h3>{translate_text("How to take good photos",lang_code)}:</h3>
        <ul>
          <li>{translate_text("Take photos in natural daylight",lang_code)}</li>
          <li>{translate_text("Capture from multiple angles",lang_code)}</li>
          <li>{translate_text("Include a scale reference if possible",lang_code)}</li>
          <li>{translate_text("Focus on the insect's distinguishing features",lang_code)}</li>
    ...
    if uploaded_file:
        st.markdown(f"""
        <div class="card">
        <h3>{translate_text("Uploaded Image Preview")}</h3>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col2:
            if st.button("🔍 Identify Insect", help="Click to identify the insect"):
                with st.spinner("Identifying insect... Please wait."):
                    ...
                    <h2>{translate_text("Identification Results",lang_code)}</h2>
                    <div style="padding: 1rem; background-color: white; border-radius: 8px; margin: 1rem 0;">
                    <h4>{translate_text("Identified Insect",lang_code)}:</h4>
                    <p style="font-size: 1.2rem; font-weight: bold; color: #2E8B57;">{translate_text(insect_label,lang_code)}</p>
                    ...
                    if insect_label in insect_control_data:
                        ...
                        with tab1:
                            ...
                            <div class="card">
                            <h4>Description</h4>
                            <p>{translate_text(insect_data['description'],lang_code)}</p>
                            
                            <h4>Risk Level</h4>
                            <p>{translate_text(insect_data['risk_level'],lang_code)}</p>
                        ...
                        with tab2:
                            ...
                            <ul>
                            {''.join([f'<li>{translate_text(pesticide,lang_code)}</li>' for pesticide in insect_data['pesticides']])}
                            </ul>
                            
                            <h4>{"Organic Control Options"}</h4>
                            <ul>
                            {''.join([f'<li>{translate_text(option,lang_code)}</li>' for option in insect_data['organic_options']])}
                            </ul>
                            </div>
                            """, unsafe_allow_html=True)
                        ...
                        with tab3:
                            ...
                            <p>{translate_text(insect_data['damage'],lang_code)}</p>
                            
                            <h4>Lifecycle Information</h4>
                            <p>{translate_text(insect_data['lifecycle'],lang_code)}</p>
                            </div>
                            """, unsafe_allow_html=True)
                        ...
                        with tab4:
                            ...
                            <ol>
                            {''.join([f'<li>{translate_text(tip,lang_code)}</li>' for tip in insect_data['tips']])}
                            </ol>
                            </div>
                            """, unsafe_allow_html=True)

                    else:
                        st.warning(f"Insect '{translate_text(insect_label,lang_code)}' not found in our database. Please consult a local agricultural extension service.")
    ...
    st.markdown(f"""
    <div class="card">
    <h1>{translate_text("Organic Farming Guide", lang_code)}</h1>
    <p>{translate_text("Step-by-step instructions for growing crops organically.", lang_code)}</p>
    </div>
    """, unsafe_allow_html=True)

    sorted_plants = sorted(organic_crops_steps.keys())
    plant = st.selectbox(translate_text("Choose a Crop", lang_code), sorted_plants)
    ...
    if plant:
        ...
        st.markdown(f"""
        <div class="card">
        <h2>{translate_text("Organic", lang_code)} {translate_text(plant, lang_code)} {translate_text("Cultivation", lang_code)}</h2>
        </div>
        """, unsafe_allow_html=True)
        
        tab1, tab2, tab3 = st.tabs([
            translate_text("📅 Growing Guide", lang_code),
            translate_text("ℹ Basic Info", lang_code),
            translate_text("💡 Tips", lang_code)
        ])
        
        with tab1:
            st.markdown(f"""
            <div class="card">
            <h3>{translate_text("Monthly Growing Guide", lang_code)}</h3>
            """, unsafe_allow_html=True)
            
            for month, steps in crop_info["steps"].items():
                st.markdown(f"**{translate_text(month, lang_code)}**")
                for step in steps:
                    st.write(f"- {translate_text(step, lang_code)}")
        ...
        with tab2:
            st.markdown(f"""
            <div class="card">
            <h3>{translate_text("Crop Information", lang_code)}</h3>
            <p><strong>{translate_text("Best Growing Season:", lang_code)}</strong> {translate_text(crop_info['month_to_grow_in_india'], lang_code)}</p>
            <p><strong>{translate_text("Ideal Weather:", lang_code)}</strong> {translate_text(crop_info['weather'], lang_code)}</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        with tab3:
            st.markdown(f"""
            <div class="card">
            <h3>{translate_text("Organic Farming Tips", lang_code)}</h3>
            <ul>
            """, unsafe_allow_html=True)
            
            for tip in crop_info["tips"]:
                st.write(f"- {translate_text(tip, lang_code)}")
    ...
    st.markdown(f"""
    <div class="card">
    <h1>{translate_text("About Vriksha Rakshak", lang_code)}</h1>
    <p>{translate_text("AI-powered agricultural assistant for sustainable farming.", lang_code)}</p>
    </div>
    """, unsafe_allow_html=True)
    
    st.markdown(f"""
    <div class="card">
    <h2>{translate_text("Our Mission", lang_code)}</h2>
    <p>{translate_text("Vriksha Rakshak aims to empower farmers with AI tools to improve crop yields, reduce losses, and promote sustainable farming practices through accessible technology.", lang_code)}</p>
    </div>
    """, unsafe_allow_html=True)
    
    st.markdown(f"""
    <div class="card">
    <h2>{translate_text("Technology Stack",lang_code)}</h2>
    ...
    st.markdown(f"""
    <div class="card">
    <h2>{translate_text("Developed", lang_code)}</h2>
    <p>{translate_text("Vriksha Rakshak was developed by a team of AI engineers from Bennett University committed to bridging technology and farming.", lang_code)}</p>
    </div>
    """, unsafe_allow_html=True)

    st.markdown(f"""
    <div class="card">
    <h2>{translate_text("Development Team", lang_code)}</h2>
    <ul>
    <li>{"Advitya Singh"}</li>
    <li>{"Shivanshi"}</li>
    <li>{"Vinith Reddy", lang_code}</li>
    ...
    st.markdown(f"""
    <div class="card">
    <h1>{translate_text("Crop Recommendation System", lang_code)}</h1>
    <p>{translate_text("Get AI-powered suggestions for the best crops to grow based on your soil and climate conditions.", lang_code)}</p>
    </div>
    """, unsafe_allow_html=True)

    # Location input
    location = st.text_input(
        translate_text("📍 Farm Location (Country/Region)", lang_code), 
        help=translate_text("Enter your country or specific region for more accurate recommendations", lang_code)
    )
    ...
    with col1:
        st.markdown(f"""
        <div class="card">
        <h3>{translate_text("Soil Parameters", lang_code)}</h3>
        """, unsafe_allow_html=True)

        N = st.number_input(translate_text("Nitrogen Content (N)", lang_code), min_value=0.0, max_value=100.0, value=50.0, 
                            help=translate_text("Typical range: 10-100 ppm", lang_code))
        P = st.number_input(translate_text("Phosphorus Content (P)", lang_code), min_value=0.0, max_value=100.0, value=50.0,
                            help=translate_text("Typical range: 10-50 ppm", lang_code))
        K = st.number_input(translate_text("Potassium Content (K)", lang_code), min_value=0.0, max_value=100.0, value=50.0,
                            help=translate_text("Typical range: 5-50 ppm", lang_code))
        ph = st.number_input(translate_text("Soil pH", lang_code), min_value=0.0, max_value=14.0, value=7.0,
                            help=translate_text("Optimal range for most crops: 6.0-7.5", lang_code))
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with col2:
        st.markdown(f"""
        <div class="card">
        <h3>{translate_text("Climate Parameters", lang_code)}</h3>
        """, unsafe_allow_html=True)

        temp = st.number_input(translate_text("Temperature (°C)", lang_code), min_value=0.0, max_value=50.0, value=25.0,
                            help=translate_text("Average temperature during growing season", lang_code))
        humidity = st.number_input(translate_text("Humidity (%)", lang_code), min_value=0.0, max_value=100.0, value=50.0,
                                help=translate_text("Average relative humidity", lang_code))
        rainfall = st.number_input(translate_text("Rainfall (mm)", lang_code), min_value=0.0, max_value=500.0, value=100.0,
                                help=translate_text("Total rainfall during growing season", lang_code))
        st.markdown("</div>", unsafe_allow_html=True)

    # Recommendation Button
    if st.button(translate_text("🌾 Find Best Crop", lang_code), help=translate_text("Click to get crop recommendation", lang_code)):
        with st.spinner(translate_text("Analyzing your parameters...", lang_code)):
            try:
                prompt = f"""
                ...
                <h2 style="color: #2E8B57;">🌱 {translate_text("AI-Powered Crop Recommendations", lang_code)}</h2>
                <div style="padding: 15px; background-color: white; border-radius: 8px; margin-top: 10px;">
                <p>{translate_text(response.text, lang_code)}</p>
                </div>
                </div>
                """, unsafe_allow_html=True)

            except Exception as e:
                st.error(translate_text(f"Error getting recommendations: {str(e)}", lang_code))
                st.markdown(f"""
                <div class="card" style="background-color: #fff0f0;">
                <h3>{translate_text("Alternative Recommendation", lang_code)}</h3>
                <p>{translate_text("While we work on fixing the AI model, here are some general recommendations based on your inputs:", lang_code)}</p>
                <ul>
                <li><strong>{translate_text("For high rainfall", lang_code)} ({rainfall}mm):</strong> {translate_text("Consider rice or taro", lang_code)}</li>
                <li><strong>{translate_text("For moderate temperatures", lang_code)} ({temp}°C):</strong> {translate_text("Wheat or maize may be suitable", lang_code)}</li>
                <li><strong>{translate_text("For your soil pH", lang_code)} ({ph}):</strong> {translate_text("Most crops prefer pH 6-7.5", lang_code)}</li>
                </ul>
                </div>
                """, unsafe_allow_html=True)
...
#         #         st.error("Sorry, we could not determine the best crop to be cultivated with the provided data.")
    ...
    st.markdown(f"""
    <div class="card">
    <h1>{translate_text("Weather Forecast",lang_code)}</h1>
    <p>{translate_text("Get real-time weather data and forecasts for better farming decisions.",lang_code)}</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if city_name:
        ...
        if current_response.status_code == 200:
            ...
            st.markdown(f"""
            <div class="card">
            <h2>{translate_text("Current Weather in",lang_code)} {city_name}, {state_name}</h2>
            </div>
            """, unsafe_allow_html=True)
            ...
            st.markdown(f"""
            <div class="card">
            <h3>{translate_text("Location Map with Weather",lang_code)}</h3>
            </div>
            """, unsafe_allow_html=True)
            ...
            if forecast_response.status_code == 200:
                ...
                st.markdown(f"""
                <div class="card">
                <h2>{translate_text("2-Day Weather Forecast",lang_code)}</h2>
                </div>
                """, unsafe_allow_html=True)
